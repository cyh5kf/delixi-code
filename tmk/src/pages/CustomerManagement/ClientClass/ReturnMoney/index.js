...
class BirthdayQueryManagement extends React.Component {
  ...
  onExport = () => {
    ...
    var str1 = this.state.timeRange.startTime.replace(/\s/g,'').slice(0,10)
    var str2 = this.state.timeRange.endTime.replace(/\s/g,'').slice(0,10)
    LoadingBar.open('导出中...')
    download('classify/collection',str1+'-'+str2+'回款用户',query).then((res)=>{
  ...
  queryBirthday = (params) => {
      LoadingBar.open()
      classManager.queryPHP('collection',params).then(res=>{
          if(res.code !==0){
              LoadingBar.close();
              Notice.open({message:res.msg})
              return false;
