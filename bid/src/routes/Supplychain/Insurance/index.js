...
import { insurancelist ,insurancedelete ,cancelInsurance ,getfile} from '../../../services/api'
...
export default class UserIntegralQuery extends PureComponent {
  ...
  async deleteing(id){
    const res = await insurancedelete({id})
    if (res&&res.success) {
      message.success('成功')
      this.getlist()      
    }
  ...
  async submit(fi){
    ...
    const res = await fetch('app/asset/currency/import',{
        method:"POST",
        body:formData
  ...
  async getlist(){
    ...
    const res = await insurancelist(params);
    if (res&&res.success) {
      this.setState({data:  res.rows,total:res.total,selectedids:[]});
  ...
  async cancelbiao(id){
    ...
    const res = await cancelInsurance(params)
    if (res.success) {
          ...
          
          {
            ...
            <div>
              ...
              <Button style={{ margin: '0 10px' }} type="primary" onClick={() => {
                ...
                <Button style={{ float: 'right' }} type="primary" onClick={()=>{this.refs.refString.click()}}>
                  导入数据
                </Button>
              <a style={{ float: 'right',marginRight:'97px' }} href="/insurance.xlsx" download='templ.xlsx'>
              下载模板
              </a>
              {/* </label> */}
              <form action=""></form>
          ...
          return (<div>
            {
              ...
              <Button type='danger' style={{marginRight:'10px'}} onClick={()=>{this.deleteing(row.id)}}>
                删除
              </Button>
              :null
            }
            <Button type='primary'  onClick={()=>{this.props.history.push(`/supplychain/insuranceinfo?id=${row.id}`)}}>
              查看详情
            </Button>
            {
              !(row.status=='0'||row.status=='2'||row.status=='5')?
              <Button type='danger' style={{marginLeft:'10px'}} onClick={()=>{this.showcancelConfirm(row.id)}}>
                撤标
              </Button>
              :null
            }
    ...
    if (list) {
      data=list.map((item,index)=>{
        const obj={
          ...
          statustxt:item.status=='0'?'处理中':item.status=='1'?'上标成功':item.status=='2'?'上标失败':item.status=='3'?'推送成功':item.status=='5'?'已撤标':'推送失败',
        }
        return obj
      })
    }
